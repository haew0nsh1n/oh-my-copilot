---
name: source-parity
description: Use when comparing OMC src implementation families with OMP source surfaces and recording parity gaps.
---

# Source Parity

Use when comparing OMC src implementation families with OMP source surfaces and recording parity gaps.
...
- Use this skill when the user asks for source parity work or when the current task naturally falls into source parity analysis.
...
- Primary OMP command: `omp source-parity [path/to/oh-my-claudecode/src]`.
- Reference alignment: OMP product skill surface.
- Runtime target: Copilot-friendly Python, CLI, markdown, `.omp` state, and repository artifacts.
- This skill also maps to a Python product skill or CLI adapter in OMP.
