...
description: Use when reminding the workflow to run local validation before completion.
---

# Local Build Reminder

Use when reminding the workflow to run local validation before completion.
...
- Use this skill when the user asks for local build reminder work or when the current task naturally falls into engineering workflow.
...
- Primary OMP command: `omp local-build-reminder <input>`.
